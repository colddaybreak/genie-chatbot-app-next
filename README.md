...
- **Genie Mode**: Back the chat UI with a Databricks AI/BI Genie space for natural language data queries (see [Genie Mode](#genie-mode))
...
Instead of a serving endpoint, the app can talk to a [Databricks AI/BI Genie](https://docs.databricks.com/aws/en/genie/) space, letting users ask natural-language questions about your data.

To enable Genie mode:

1. Create a Genie space in your workspace and copy its space ID from the URL (`https://<workspace-host>/genie/spaces/<space-id>`).
2. In `databricks.yml`, set the `genie_space_id` and `genie_space_name` variables (e.g. `databricks bundle deploy --var genie_space_id=<space-id> --var genie_space_name=<name>`).
3. Local development: set `DATABRICKS_GENIE_SPACE_ID` in `.env` (see `.env.example`).

When `DATABRICKS_GENIE_SPACE_ID` is set the server:

- Routes every chat turn through the configured Genie space using the Genie REST API, with OBO user scopes (`dashboards.genie`).
- Streams Genie message statuses (fetching metadata, executing query, ...) as reasoning parts and renders SQL queries plus result tables inline as markdown.
- Generates chat titles locally (no extra Genie round-trip).
- Stays in memory mode for chat history unless the optional database resources are enabled; feedback is disabled in Genie mode because Genie responses do not carry MLflow traces.
