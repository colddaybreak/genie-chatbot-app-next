...
import {
  createGenieLanguageModel,
  createGenieTitleModel,
  isGenieConfigured,
} from './genie-provider';
...
export class OAuthAwareProvider implements SmartProvider {
  ...
  async languageModel(id: string): Promise<LanguageModelV3> {
    // Genie mode: back the chat UI with a Genie space instead of a serving
    // endpoint. Title/artifact generation uses a local model so we don't burn
    // a Genie round-trip on every new conversation.
